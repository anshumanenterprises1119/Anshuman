...
console.log(`Auditing and upgrading ${htmlFiles.length} HTML files...`);
...
htmlFiles.forEach(filePath => {
  ...
  const isSubFolder = filePath.includes('\\contractor\\') || filePath.includes('\\decoratenow\\');
  const cssPrefix = isSubFolder ? '../' : '';
  const jsPrefix = isSubFolder ? '../' : '';
  const isDecorateNow = filePath.includes('decoratenow');
  const isContractor = filePath.includes('contractor');

  // 1. Ensure CSS link for ae-responsive.css is present before </head>
  if (!content.includes('ae-responsive.css')) {
    const cssTag = `<link rel="stylesheet" href="${cssPrefix}css/ae-responsive.css"/>\n</head>`;
    content = content.replace('</head>', cssTag);
  }

  // 2. Ensure JS script for ae-mobile-nav.js is present before </body>
  if (!content.includes('ae-mobile-nav.js')) {
    const jsTag = `<script src="${jsPrefix}js/ae-mobile-nav.js"></script>\n</body>`;
    if (content.includes('</body>')) {
      content = content.replace('</body>', jsTag);
    } else {
      content += `\n<script src="${jsPrefix}js/ae-mobile-nav.js"></script>`;
    }
  }

  // 3. Remove duplicate header or orphan fragments
  const headerOrphanRegex = /<div style="display:flex;align-items:center;gap:16px;color:#cbd5e1;font-size:11px;font-weight:500;white-space:nowrap;"[\s\S]*?<\/header>\s*<\/div>/gi;
  content = content.replace(headerOrphanRegex, '');

  // 4. Construct active hub styles & links
  let activeHub = 'b2b';
  ...
  const logoImg = isDecorateNow ? `${cssPrefix}decoratenow-logo.png` : (isContractor ? `${cssPrefix}aditya-contractor-logo.png` : `${cssPrefix}logo.webp`);
  const brandTitle = isDecorateNow ? 'DecorateNow' : (isContractor ? 'Aditya Contractor' : 'Anshuman Enterprises');
  const homeLink = isDecorateNow ? `${cssPrefix}decoratenow/index.html` : (isContractor ? `${cssPrefix}contractor/index.html` : `${cssPrefix}index.html`);
  const aboutLink = `${cssPrefix}about.html`;
  const contactLink = isDecorateNow ? `${cssPrefix}decoratenow/contact.html` : (isContractor ? `${cssPrefix}contractor/contact.html` : `${cssPrefix}contact.html`);
  const faqLink = isDecorateNow ? `${cssPrefix}decoratenow/faq.html` : (isContractor ? `${cssPrefix}contractor/faq.html` : `${cssPrefix}faq.html`);
  const catalogueLink = isDecorateNow ? `${cssPrefix}decoratenow/products.html` : (isContractor ? `${cssPrefix}contractor/services.html` : `${cssPrefix}our-catalogue.html`);
  const productsLink = isDecorateNow ? `${cssPrefix}decoratenow/products.html` : `${cssPrefix}products.html`;
...
<div id="ae-sticky-wrap">
  <div id="ae-global-switcher" style="background:#0f172a;padding:8px 16px;border-bottom:1px solid rgba(255,255,255,0.1);font-family:'Montserrat',sans-serif;">
    <div style="max-width:1280px;margin:0 auto;display:flex;align-items:center;justify-content:space-between;gap:16px;">
      <div style="display:flex;align-items:center;gap:8px;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none;">
        <a href="${cssPrefix}index.html" style="display:inline-flex;align-items:center;gap:6px;padding:5px 14px;border-radius:50px;text-decoration:none;font-weight:700;font-size:12px;white-space:nowrap;background:${hubBg};color:${hubColor};">🏢 Anshuman Hub</a>
        <a href="${cssPrefix}decoratenow/index.html" style="display:inline-flex;align-items:center;gap:6px;padding:5px 14px;border-radius:50px;text-decoration:none;font-weight:700;font-size:12px;white-space:nowrap;background:${dnBg};color:${dnColor};">💡 DecorateNow</a>
        <a href="${cssPrefix}contractor/index.html" style="display:inline-flex;align-items:center;gap:6px;padding:5px 14px;border-radius:50px;text-decoration:none;font-weight:700;font-size:12px;white-space:nowrap;background:${acBg};color:${acColor};">🔧 Aditya Contractor</a>
      </div>
  ...
  <div id="ae-cat-nav" style="background:#FAF7F2;border-bottom:1px solid rgba(201,168,76,0.4);padding:8px 16px;overflow:hidden;">
    <div style="max-width:1280px;margin:0 auto;display:flex;align-items:center;gap:8px;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none;">
      <a href="${cssPrefix}electrical-contracting.html" style="display:inline-flex;align-items:center;gap:4px;padding:5px 14px;border-radius:50px;text-decoration:none;font-size:12px;font-weight:700;white-space:nowrap;background:rgba(201,168,76,0.18);border:1px solid rgba(201,168,76,0.5);color:#C9A84C;">⚡ Electrical</a>
      <a href="${cssPrefix}cctv-installation.html" style="display:inline-flex;align-items:center;gap:4px;padding:5px 14px;border-radius:50px;text-decoration:none;font-size:12px;font-weight:700;white-space:nowrap;background:rgba(201,168,76,0.18);border:1px solid rgba(201,168,76,0.5);color:#C9A84C;">📹 CCTV & Security</a>
      <a href="${cssPrefix}wires-cables.html" style="display:inline-flex;align-items:center;gap:4px;padding:5px 14px;border-radius:50px;text-decoration:none;font-size:12px;font-weight:700;white-space:nowrap;background:rgba(201,168,76,0.18);border:1px solid rgba(201,168,76,0.5);color:#C9A84C;">🔌 Wires & Cables</a>
      <a href="${cssPrefix}led-lighting.html" style="display:inline-flex;align-items:center;gap:4px;padding:5px 14px;border-radius:50px;text-decoration:none;font-size:12px;font-weight:700;white-space:nowrap;background:rgba(201,168,76,0.18);border:1px solid rgba(201,168,76,0.5);color:#C9A84C;">💡 LED Lighting</a>
      <a href="${cssPrefix}smart-door-locks.html" style="display:inline-flex;align-items:center;gap:4px;padding:5px 14px;border-radius:50px;text-decoration:none;font-size:12px;font-weight:700;white-space:nowrap;background:rgba(201,168,76,0.18);border:1px solid rgba(201,168,76,0.5);color:#C9A84C;">🔒 Smart Security</a>
      <a href="${cssPrefix}network-rack-setup.html" style="display:inline-flex;align-items:center;gap:4px;padding:5px 14px;border-radius:50px;text-decoration:none;font-size:12px;font-weight:700;white-space:nowrap;background:rgba(201,168,76,0.18);border:1px solid rgba(201,168,76,0.5);color:#C9A84C;">🌐 Networking</a>
...
`;
  ...
  // Replace existing header wrap with updated responsive code
...
console.log(`Successfully upgraded mobile menu integration across ${totalFixed} HTML files!`);
